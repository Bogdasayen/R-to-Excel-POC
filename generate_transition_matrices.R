...
#' @export
markov_model$set("public", "generate_transition_matrices", function() {
  # One 4x4 transition matrix for each cycle, implant_name and sample.
  transition_matrices <- array(
    0,
    dim = c(n_cycles, n_implants, n_samples, n_states, n_states),
  ...
  uk_lifetables <- read_excel(
    paste0(data_directory, "/hips_input_data.xlsx"),
    sheet = "uk_lifetables"
  )
  ...
  for (i_cycle in 1:n_cycles) {
    for (implant_name in implant_names) {
      transition_matrices[
        ...
        1 -
        exp(
          -exp(input_parameters[, paste0(
            "log_rate_1st_revision_",
            implant_name
          )])
        )
      transition_matrices[
      ...
      transition_matrices[i_cycle, implant_name, , "post_thr", "dead"] <-
        transition_matrices[i_cycle, implant_name, , "post_1st_rev", "dead"] <-
          transition_matrices[
            ...
            1 -
            exp(-with(uk_lifetables, Males[Age = initial_age + i_cycle - 1]))
      ...
      for (i_state in 1:length(state_names)) {
        transition_matrices[i_cycle, implant_name, , i_state, i_state] <- 1 -
          apply(
            transition_matrices[i_cycle, implant_name, , i_state, -i_state],
            c(1),
            sum,
            na.rm = TRUE
          )
  ...
  if (
    prod(
      rowSums(transition_matrices[
        sample(1:n_cycles, 1),
        sample(implant_names, 1),
        sample(1:n_samples, 1),
        ,
      ]) ==
        1
    ) !=
