...
summarise_samples <- function(v_samples, n_digits = 3) {
  return(paste0(format(mean(v_samples), digits = n_digits, nsmall = n_digits),
                " (", format(quantile(v_samples, probs = 0.025), digits = n_digits, nsmall = n_digits),
                ", ", format(quantile(v_samples, probs = 0.975), digits = n_digits, nsmall = n_digits), ")"))
}